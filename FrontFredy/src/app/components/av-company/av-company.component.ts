...
export const AV_INPUT_CONTROL_VALUE_ACCESSOR: any = {
  ...
  useExisting: forwardRef(() => AvCompanyComponent),
  multi: true,
};

@Component({
  selector: 'av-company',
  templateUrl: './av-company.component.html',
  providers: [AV_INPUT_CONTROL_VALUE_ACCESSOR],
})

export class AvCompanyComponent implements OnInit {
  @Input() label: string;
  ...
  @Input() selectedCompanys: Company[];
  @Input() selectioncompanysAvailable: Company;
  @Input() control: FormControl;
  @Output() sendCompany: EventEmitter<Company> = new EventEmitter<Company>();
  ...
  getCitysByName(query: String) {
    this.transactionService.GetList(environment.CBPAPI, this.constantService.COMPANY_URL, '/GetCompanyByName/' + query).subscribe(
      (data) => {
  ...
  writeValue(value: any): void {
    this.selectioncompanysAvailable = value;
  }
  ...
  onClear() {
    this.selectioncompanysAvailable = new Company();
    this.sendCompany.emit();
  }
}
