...
export const AV_INPUT_CONTROL_VALUE_ACCESSOR: any = {
  ...
  useExisting: forwardRef(() => AvMeasurementUnitTypeComponent),
  multi: true,
};

@Component({
  selector: 'av-measurement-unit-type',
  templateUrl: './av-measurement-unit-type.component.html',
  providers: [AV_INPUT_CONTROL_VALUE_ACCESSOR],
})
export class AvMeasurementUnitTypeComponent implements ControlValueAccessor {
  @Input() label: string;
  ...
  @Output() sendDesc: EventEmitter<String> = new EventEmitter<String>();
  ...
  getLengthUnits() {
    this.transactionService.GetList(environment.adminAPI, this.constantService.MEASUREMENT_UNIT_URL, '/GetMeasurement').subscribe(
      (data) => {
  ...
  onChange(): void {
    this.zone.run(() => {
      ...
      this.sendDesc.emit(description[0].label);
    });
  }
