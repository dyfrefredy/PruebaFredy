...
export const AV_INPUT_CONTROL_VALUE_ACCESSOR: any = {
    ...
    useExisting: forwardRef(() => AvProjectTypeComponent),
    multi: true,
};

@Component({
    selector: 'av-project-type',
    templateUrl: './av-project-type.component.html',
    providers: [AV_INPUT_CONTROL_VALUE_ACCESSOR],
})
export class AvProjectTypeComponent implements ControlValueAccessor {
    @Input() label: string;
    ...
    getProjectType() {
        ...
        this.transactionService.getPaginationAndFilter(environment.adminAPI, this.constantService.PROJECT_TYPE_URL, filter).subscribe(
            (data) => {
                if (data.responseDto.response === this.constantService.RESPONSE_OK) {
                    this.options = new Array();
                    this.options.push({ label: 'Seleccione tipo de proyecto', value: 0 });
