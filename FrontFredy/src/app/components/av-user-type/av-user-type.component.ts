...
export const AV_INPUT_CONTROL_VALUE_ACCESSOR: any = {
  ...
  useExisting: forwardRef(() => AvUserTypeComponent),
  multi: true,
};

@Component({
  selector: 'av-user-type',
  templateUrl: './av-user-type.component.html',
  providers: [AV_INPUT_CONTROL_VALUE_ACCESSOR],
})

export class AvUserTypeComponent implements ControlValueAccessor {
  @Input() label: string;
  ...
  getUserTypes(){
    this.transactionService.getAll(environment.adminAPI, this.constantService.USER_TYPE_URL).subscribe(
      (data) => {
        if (data.responseDto.response === this.constantService.RESPONSE_OK) {
          this.options = new Array();
          this.options.push({ label:  'Seleccione tipo de usuario', value: 0 });
  ...
  writeValue(value: any): void {
    this.selectionUserType = value;
  }
  onChange(): void {
    this.zone.run(() => {
      this.propagateChange(this.selectionUserType);
    });
  }
